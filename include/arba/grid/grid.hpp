...
public:
    ...
    inline const_iterator iterator_at(const GridPosition auto& position) const
    {
        return begin() + (position.y() * width() + position.x());
    }
    inline iterator iterator_at(const GridPosition auto& position)
    {
        return begin() + (position.y() * width() + position.x());
    }
